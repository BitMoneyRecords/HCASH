...
#include "ui_zHCASHcontroldialog.h"
...
std::list<std::string> ZHCASHControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZHCASHControlDialog::listMints;

ZHCASHControlDialog::ZHCASHControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZHCASHControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZHCASHControlDialog::~ZHCASHControlDialog()
{
    delete ui;
}

void ZHCASHControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHCASHControlDialog::updateList()
{
...
void ZHCASHControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHCASHControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZHCASH_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHCASHControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZHCASHControlDialog::GetSelectedMints()
{
...
void ZHCASHControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
