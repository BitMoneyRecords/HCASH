...
*Setup instructions for a gitian build of HCASH using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the HCASH
...
Clone the git repositories for HCASH and gitian and then checkout the HCASH version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the HCASH repository
...
To build HCASH (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the HCASH repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/HCASH/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/HCASH
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/HCASH.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/HCASH-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/HCASH-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/HCASH-build.assert
```
